...
func ProcessSongRequestSpotify(irc *IRCConn, channel string, permissionLevel int, brokenMsg []string) {
	...
	if TrackID == "" {
		ctx := context.Background()
		result, err := state.SpotifyClient.Search(ctx, strings.Join(brokenMsg[1:], " "), spotify.SearchTypeTrack, spotify.Market("US"), spotify.Limit(1)) // TODO: use target profile country code
		if err != nil {
